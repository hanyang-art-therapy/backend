...
//                    .flatMap(gallery ->
// artsRepository.findByGalleriesNo(gallery.getGalleriesNo()).stream())
...
//                                      .orElseThrow(() -> new IllegalArgumentException("해당 전시회가
// 존재하지 않습니다."));
//
...
//                                            .filter(arts ->
// arts.getGalleriesNo().equals(gallery.getGalleriesNo())))
//                    .collect(Collectors.toList());
//
//    return mapToDtoList(artsList, LocalDate.now().getYear());
//  }
...
// artist.getStudentNo(), artist.getCohort()))
//                              .orElseThrow(() -> new EntityNotFoundException("작가 정보를 찾을 수
// 없습니다."));
//                    })
//            .collect(Collectors.toList());
//  }
